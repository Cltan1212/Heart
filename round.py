...
# Time library is imported, which helps load time module, as it provides various time-related functions
import time
...
class Round:
    """Round class for representing and manipulating each round in hearts game"""
    ...
    def __init__(self, players: list[Player]) -> None:
        ...
        self.broken_hearts = False              # Broken_hearts is initailise to false, it will change to true when hearts is broken
        self.leader = self.initiate()
        for _ in range(len(players[0].hand)):   # Round continue after player plays all card
            self.execution()

    # Define a method which use to initialise who is the leader(who take the Two of Clubs)
    def initiate(self) -> int:
        ...
        # By using for loop to find who is the leader in which the player takes Two of Clubs card
    ...
    def execution(self) -> None:
        ...
        trick = []              # Will be used to store the cards of the player
        ...
        post_calc = self.players[self.leader].round_score                                               # Score that leader received so far (including previous round score)
